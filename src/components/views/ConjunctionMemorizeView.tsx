...
export function ConjunctionMemorizeView() {
  const navigate = useNavigate();
  const { conjunctionMemorize } = useAppContext();
  const {
    phase,
    queue,
    quizQueue,
  ...
  if (phase === 'results' || isComplete) {
    return <ResultPhase sessionResults={sessionResults} onBack={handleBack} itemNoun="접속사" />;
  }
