...
export function HomeView({ onOpenSettings, onOpenMemorizedList }: HomeViewProps) {
  ...
  const { auth, vocabulary, grammarStats, memorize, solve, vocabQuiz } = useAppContext();
  const { user, login, logout } = auth;
  const { words, memorizedCount, totalLearningDays, monthlyActivity } = vocabulary;
          ...
          {user ? (
            <>
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2 bg-white p-2 pr-4 rounded-2xl border border-slate-100 shadow-sm">
                  <img src={user.photoURL || ''} alt="" className="w-8 h-8 rounded-xl bg-slate-100" referrerPolicy="no-referrer" />
                  <span className="text-xs font-bold text-slate-700">{user.displayName}</span>
                </div>
              </div>
              <button onClick={logout} className="text-[10px] font-bold text-slate-400 hover:text-rose-500 flex items-center gap-1 transition-colors">
            ...
            <button 
              onClick={login}
              className="flex items-center gap-2 bg-white px-4 py-2 rounded-2xl border border-slate-100 shadow-sm hover:bg-slate-50 transition-all text-sm font-bold text-slate-700"
            >
              <LogIn className="w-4 h-4 text-blue-600" /> 로그인
            </button>
          )}
        </div>
      </div>
      ...
      {!user ? (
        <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 text-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="font-bold text-blue-900">로그인하여 시작하기</h3>
            <p className="text-xs text-blue-700 leading-relaxed">학습 진행 상황과 단어장이 모든 기기에 동기화됩니다.</p>
          </div>
          <button 
            onClick={login}
